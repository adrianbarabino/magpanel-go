...
func (h *HandlersStruct) LoginUser(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.dataBase.SelectRow("SELECT id, username, password_hash, salt FROM users WHERE username = ?", loginData.Username)
	if err != nil {
...
func (h *HandlersStruct) RequestPasswordRecovery(w http.ResponseWriter, r *http.Request) {
	...
	var u models.User
	rows, err := h.dataBase.SelectRow("SELECT id, email FROM users WHERE email = ?", requestData.Email)
	if err != nil {
	...
	_, err = h.dataBase.Update(true, "UPDATE users SET recovery_hash = ?, recovery_hash_time = NOW() WHERE id = ?", recoveryToken, u.ID)
...
func (h *HandlersStruct) ChangePassword(w http.ResponseWriter, r *http.Request) {
	var requestData struct {
	...
	rows, err := h.dataBase.SelectRow("SELECT id, recovery_hash_time FROM users WHERE recovery_hash = ?", requestData.Token)
	if err != nil {
	...
	_, err = h.dataBase.Update(true, "UPDATE users SET password_hash = ?, salt = ?, recovery_hash = NULL WHERE id = ?", newHashedPassword, newSalt, userID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
